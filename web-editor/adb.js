...
(() => {
  ...
  // The File System Access API's directory/file pickers refuse to run in any
  // embedded (non top-level) browsing context - e.g. VS Code's integrated
  // "Simple Browser" panel, which loads pages inside an iframe - and throw a
  // SecurityError instead. window.top is only readable when same-origin, so
  // treat a cross-origin access error as "embedded" too.
  ...
  function showUnsupportedBanner(message, action) {
    el.unsupportedBanner.style.display = 'flex';
    el.unsupportedBanner.innerHTML = '';
    const span = document.createElement('span');
    span.textContent = message;
    el.unsupportedBanner.appendChild(span);
    if (action) {
      const btn = document.createElement('button');
      btn.textContent = action.label;
      btn.addEventListener('click', action.onClick);
      el.unsupportedBanner.appendChild(btn);
    }
  }
  ...
  function checkSupport() {
    ...
    if (isEmbeddedFrame()) {
      showUnsupportedBanner(
        'This editor cannot access local files while running inside an embedded browser frame, such as ' +
        'VS Code\'s "Simple Browser" panel. Open this page in a full browser tab instead.',
        { label: 'Open in Browser Tab', onClick: () => window.open(window.location.href, '_blank') }
      );
      el.btnOpenRepo.disabled = true;
      el.btnBindFolder.disabled = true;
      return false;
    }
    return true;
  }
  ...
  async function openRepo() {
    try {
      ...
      if (err.name === 'SecurityError') {
        setStatus(
          'Failed to open repository: file pickers are blocked in this embedded browser frame ' +
          '(e.g. VS Code\'s Simple Browser). Open this page in a full browser tab instead.',
          true
        );
        return;
      }
      setStatus('Failed to open repository: ' + err.message, true);
    }
  }
  ...
  async function bindFolder() {
    try {
      ...
      if (err.name === 'SecurityError') {
        setStatus(
          'Failed to bind folder: file pickers are blocked in this embedded browser frame ' +
          '(e.g. VS Code\'s Simple Browser). Open this page in a full browser tab instead.',
          true
        );
        return;
      }
      setStatus('Failed to bind folder: ' + err.message, true);
    }
  }
  ...
  function renderPlainEditor(record, container, paneId) {
    ...
    updateLineNumbers(gutter, record.content);
